...
def random_crop_and_resize_image_and_labels(image, label, crop_h, crop_w, ignore_label=255):
    ...
    nh = tf.to_int32(tf.maximum(crop_h, image_shape[1]))
    nw = tf.to_int32(tf.maximum(crop_w, image_shape[2]))
    image = tf.image.resize_bilinear(image, [nh, nw])
    label = tf.image.resize_nearest_neighbor(label,[nh, nw])

    combined_pad = tf.concat(axis=3, values=[image, label])
