...
def deeplabv3(inputs,
  ...
  """
  ih, iw = tf.shape(inputs)[1], tf.shape(inputs)[2]
  ...
  with tf.variable_scope(scope, [inputs], reuse=reuse) as sc:
    ...
    with slim.arg_scope(resnet_arg_scope(weight_decay=args.weight_decay, batch_norm_decay=args.bn_weight_decay)):
      with slim.arg_scope([slim.conv2d, bottleneck],outputs_collections=end_points_collection):
        with slim.arg_scope([slim.batch_norm], is_training=is_training):
          net = inputs

          short_cut_1 = resnet_utils.conv2d_same(net, 32, 3, stride=1, scope='short_cut_1.1')
          short_cut_1 = resnet_utils.conv2d_same(short_cut_1, 32, 3, stride=1, scope='short_cut_1.2')
          short_cut_1 = slim.utils.collect_named_outputs(end_points_collection, sc.name, short_cut_1)
          ...
          with tf.variable_scope('block4', [net]) as sc:
            ...
            for i in range(3):
              with tf.variable_scope('unit_%d' % (i + 1), values=[net]):
                net = bottleneck(net, depth=base_depth * 4, depth_bottleneck=base_depth, stride=1, rate=2*multi_grid[i])
            net = slim.utils.collect_named_outputs(end_points_collection, sc.name, net)
          ...
          if aspp:
            with tf.variable_scope('aspp', [net]) as sc:
              ...
              for i in range(len(rate_lst)):
                branch_2 = slim.conv2d(net, 256, [3,3], stride=1, rate=rate_lst[i], scope='rate{}'.format(6*(i+1)))
            ...
            with tf.variable_scope('fusion', [aspp_list, pooled]) as sc:
              ...
              net = slim.conv2d(net, 32, [1, 1], stride=1, scope='1x1conv64')
              net = slim.utils.collect_named_outputs(end_points_collection, sc.name, net)
              net = tf.image.resize_bilinear(net, [ih, iw])

              net = tf.concat([net, short_cut_1],3)
              net = slim.utils.collect_named_outputs(end_points_collection, sc.name, net)
          else:
